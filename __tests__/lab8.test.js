describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    const prodLength = prodItems.length;
    for (let i = 0; i < prodLength; i += 1){
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let piShadowRoot = await productItem.getProperty('shadowRoot');
    let button = await piShadowRoot.$('button');
    let buttonInnerText = await button.getProperty('innerText');
    let textValue = await buttonInnerText['_remoteObject'].value;
    expect(textValue).toBe('Add to Cart');
    await button.click();
    buttonInnerText = await button.getProperty('innerText');
    textValue = await buttonInnerText['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < productLength; i += 1){
      let piShadowRoot = await productItems[i].getProperty('shadowRoot');
      let button = await piShadowRoot.$('button');
      await button.click();
    }
    let cartCount = await page.$('#cart-count');
    let cartCountInnerText = await cartCount.getProperty('innerText');
    let textValue = await cartCountInnerText['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let cartCount = await page.$('#cart-count');
    let cartCountInnerText = await cartCount.getProperty('innerText');
    let carttextValue = await cartCountInnerText['_remoteObject'].value;
